...
sh "/etc/network/if-down.d/client-network-autoUNmount.sh"

logger -t mountsshfs "Beginning client-network-automount"
...
do
    ...
    # check if the line is a selected line
        ...
        if am_i "$mp_uid"; then
                        ...
                        { sh -c "mount $mp" &&
                                logger -t mountsshfs "$mp mounted as current user (`id -un`) with $opts" ||
                                logger -t mountsshfs "$mp failed to mount as current user (`id -un`) with $opts";
                        } &
                elif am_i root; then
                        ...
                        { sudo -u "$mp_uid" sh -c "mount $mp" &&
                                logger -t mountsshfs "$mp mounted as $mp_uid with $opts" ||
                                logger -t mountsshfs "$mp failed to mount as $mp_uid with $opts";
                        } &
                else
                        # otherwise, don't try to mount another user's mount point
                        logger -t mountsshfs "Not attempting to mount $mp as other user $mp_uid"
:
                        logger -t mountsshfs "Not attempting to mount $mp as other user $mp_uid"
                fi
    fi
